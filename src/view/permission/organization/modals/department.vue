...
<script>
import { organizationOperate, getOrganizationInfo, getOrganizationLeaders } from "api/permission";
export default {
  data () {
    return {
      dialogTitle: '添加子部门',
    ...
    onSave () {
      this.$refs.paramForm.validate(valid => {
        if (valid) {
          ...
          params['documentCode'] = `${params.documentCode.pre}-${params.documentCode.cur}-${params.documentCode.next}`;
          if (this.type == 'add') {
            params.parentId = this.id;
          }
          organizationOperate({organizationGroup: params}, this.type).then(() => {
  ...
  created () {
    ...
    if (this.type == 'edit') {
      this.bLoading = true;
      getOrganizationInfo({organizationGroupId: this.id}).then(data => {
