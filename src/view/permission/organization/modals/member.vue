  ...
  <el-dialog :title="dialogTitle" top="5vh" :visible.sync="bShow" :before-close="onClose">
    ...
    <el-form :model="form" :rules="rules" v-loading="bLoading"
      ...
      <el-row>
        ...
        <el-form-item label="角色" prop="organizationRoleId">
          <el-select v-model="form.organizationRoleId"
...
<script>
import { membersOperate, getMemberInfo } from "api/permission";
export default {
  data () {
    return {
      dialogTitle: '新增人员',
  ...
  watch: {
    ...
    'userId': {
      ...
      handler (val) {
        ...
        if (val) {
          getMemberInfo({organizationGroupId: this.id, userId: val}).then(data => {
            Object.assign(this.form, data.user);
            this.form.organizationRoleId = data.organizationRoleList[0] && data.organizationRoleList[0].organizationRoleId;
          });
        }
      }
    }
    ...
    onSave () {
      this.$refs.paramForm.validate(valid => {
        if (valid) {
          ...
          membersOperate(finalParams, this.userId ? 'edit' : 'add').then(() => {
